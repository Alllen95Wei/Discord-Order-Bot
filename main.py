...
async def on_message(message):
    global final_msg, time_stamp, mode, current_order
    ...
    if message.author == client.user:
        ...
        if msg_in[3:] == "ping":
            embed = discord.Embed(title="延遲", description="{0}ms".format(str(round(client.latency * 1000))), color=0x14E073)
            embed.set_footer(text=time_stamp)
            final_msg.append(embed)
        if msg_in[3:6] == "add":
            if "busy" not in mode:
                ...
                embed.set_footer(text=time_stamp)
                final_msg.append(embed)
    elif msg_in.startswith("http://") or msg_in.startswith("https://"):
        if mode == "busy-1":
            mode = "busy-2"
        ...
        if mode == "busy-2":
            ...
            embed.set_footer(text=time_stamp)
            final_msg.append(embed)
            embed = discord.Embed(title="訂單內容-" + str(current_order["order_num"]), description="商品網址：" +
                                  str(current_order["url"]) + "\n商品數量：" + str(current_order["amount"]), color=0x14E073)
            embed.set_footer(text=time_stamp)
            final_msg.append(embed)
            embed = discord.Embed(title="確認訂單", description="請輸入`y`表示**確認**。或是輸入`n`，以取消這次訂單。", color=0x14E073)
            final_msg.append(embed)
        else:
